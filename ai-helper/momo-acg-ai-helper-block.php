 ...
 * @since v3.5.0
 */
class Momo_ACG_AI_Helper_Block {
	/**
	 * Constructor
	 */
	...
	public function momo_acg_ai_helper_block_init() {
		...
		$settings = array(
			'rest_aihelper'           => get_rest_url() . 'momoacg/v1/aihelper',
	...
	public function momo_acg_aihelper_post_handler( $request ) {
		...
		if ( 'initial' === $type ) {
			...
			$content   = '';
			if ( 'longer' === $prompt ) {
				$content = esc_html__( 'Modify this article to convey a greater length or expanded meaning.', 'momoacg' );
			} elseif ( 'shorter' === $prompt ) {
				$content = esc_html__( 'Modify this piece of writing using fewer words.', 'momoacg' );
			}
			$new_msg   = array(
				'role'    => 'user',
				'content' => $content,
