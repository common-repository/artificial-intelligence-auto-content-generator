...
jQuery(document).ready(function ($) {
    ...
    $('body').on('click', '#momo_be_mb_openai_generate_content_fe', function (e) {
        e.preventDefault();
        var $parent = $(this).closest('#openai-content-generator');
        var $working = $parent.find('.momo-be-working');
        var $msgBox = $parent.find('#momo_be_mb_openai_messagebox');
        ...
        ajaxdata.security = momoacgcs_fe.momoacgcs_ajax_nonce;
        ajaxdata.action = 'momo_acg_openai_generate_content_fe';
        $.ajax({
            beforeSend: function () {
                $contentBox.val('');
                $working.addClass('show');
                $msgBox.html(momoacgcs_fe.generating_content);
                $msgBox.show();
            },
            type: 'POST',
            dataType: 'json',
