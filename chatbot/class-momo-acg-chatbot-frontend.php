 ...
 * @since v3.10.0
 */
class MoMo_ACG_Chatbot_Frontned {
	/**
	 * Unique session
	 *
	 * @var string
	...
	public function momo_acg_chatbot_post_handler( $request ) {
		...
		$default_icon     = $momoacg->plugin_url . 'chatbot/assets/images/chatbot.png';
		...
		$ai_name          = isset( $chatbot_settings['ai_name'] ) && ! empty( $chatbot_settings['ai_name'] ) ? $chatbot_settings['ai_name'] : esc_html__( 'AI', 'momoacg' );
		$welcome_message  = isset( $chatbot_settings['welcome_message'] ) && ! empty( $chatbot_settings['welcome_message'] ) ? $chatbot_settings['welcome_message'] : esc_html__( 'How can I help you?', 'momoacg' );
		$username         = isset( $chatbot_settings['username'] ) && ! empty( $chatbot_settings['username'] ) ? $chatbot_settings['username'] : esc_html__( 'User', 'momoacg' );

		$casually_fine_tuned = $momoacg->fn->momo_return_option_yesno( $chatbot_settings, 'casually_fine_tuned' );

		$enable_ft_chatbot_modal   = $momoacg->fn->momo_return_option_yesno( $chatbot_settings, 'enable_ft_chatbot_modal' );
		...
		if ( empty( $question ) ) {
			ob_start();
			$empty_question = esc_html__( 'Empty question provided. Please try again.', 'momoacg' );
			?>
		...
		if ( 'on' === $enable_ft_chatbot_modal && ! empty( $selected_ft_chatbot_modal ) ) {
			...
			/**** For new chatbot ******** */
			$training_settings = get_option( 'momo_acg_cb_trainings_settings' );
			if (
			...
			$content = array(
				'role'    => 'assistant',
				'content' => esc_html__( 'Please donot provide any answer as an AI, meaning pretending as an AI ( Chatbot, Digital Assistant ), act as a Human ( answer it as if user is talking to a human ), please pretend as Human also check context before answering.\\n\\n If answering from context please do not say as youve read it from above, answer it as if you\'ve got knowledge about it. If there\'s a context with __MOMO_CHATBOT_EMBEDDINGS__, then prepare your answer only based on that context.', 'momoacg' ),
			);
	...
	public function momo_acg_cb_load_scripts_styles() {
		...
		wp_enqueue_style( 'momoacg_chatbot_style', $momoacg->plugin_url . 'chatbot/assets/momo_acg_chatbot.css', array(), $momoacg->version );
		wp_register_script( 'momoacg_chatbot_script', $momoacg->plugin_url . 'chatbot/assets/momo_acg_chatbot.js', array( 'jquery' ), $momoacg->version, true );
		...
		$ajaxurl = array(
			...
			'username'             => $username,
			'rest_endpoint'        => get_rest_url() . 'momoacg/v1/chatbot',
			'max_length'           => (int) $max_length,
			'typing'               => ( '' === $typing ) ? esc_html__( 'Collecting data', 'momoacg' ) : $typing,
		);
	...
	public function momo_acg_generate_chatbot( $attributes, $chatbot_cookie ) {
		...
		$welcome_message  = isset( $chatbot_settings['welcome_message'] ) && ! empty( $chatbot_settings['welcome_message'] ) ? $chatbot_settings['welcome_message'] : esc_html__( 'How can I help you?', 'momoacg' );
		$username         = isset( $chatbot_settings['username'] ) && ! empty( $chatbot_settings['username'] ) ? $chatbot_settings['username'] : esc_html__( 'User', 'momoacg' );
		$placeholder      = isset( $chatbot_settings['placeholder'] ) && ! empty( $chatbot_settings['placeholder'] ) ? $chatbot_settings['placeholder'] : esc_html__( 'Type your message here ....', 'momoacg' );
		$position         = isset( $chatbot_settings['position'] ) ? $chatbot_settings['position'] : 'bright';
		$popup            = isset( $chatbot_settings['popup'] ) && ! empty( $chatbot_settings['popup'] ) ? $chatbot_settings['popup'] : esc_html__( 'Chatbot', 'momoacg' );
	...
	public function momo_chatbot_log_transient( $content ) {
		...
		if ( empty( $chatbot_session ) ) {
			...
			$momoacg->embeddings->momo_log_transient_to_option( $transient_id, 'new' );
		}
		if ( isset( $content['role'] ) && 'system' === $content['role'] ) {
			$momoacg->embeddings->momo_log_transient_to_option( $transient_id, 'old' );
		}
	...
	public function momo_get_post_content_by_id_index( $post_id_with_index ) {
		...
		$post_id   = (int) $parts[0];
		$index     = (int) $parts[1];
		$fragments = $momoacg->embeddings->momo_get_post_sentence_fragments( $post_id );
